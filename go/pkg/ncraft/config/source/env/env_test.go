...
func TestEnvvar_Prefixes(t *testing.T) {
    ...
    var prefixtests = []struct {
        prefixOpts   []source2.Option
        expectedKeys []string
    }{
        {[]source2.Option{WithPrefix("APP", "MICRO")}, []string{"app", "micro"}},
        {[]source2.Option{WithPrefix("MICRO"), WithStrippedPrefix("APP")}, []string{"database", "micro"}},
        {[]source2.Option{WithPrefix("MICRO"), WithStrippedPrefix("APP")}, []string{"database", "micro"}},
    }
...
func TestEnvvar_WatchNextNoOpsUntilStop(t *testing.T) {
    ...
    if _, err := w.Next(); err != source2.ErrWatcherStopped {
